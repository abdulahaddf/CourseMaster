...
export default function EditCoursePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        ...
        const formattedBatches = data.course.batches?.map((batch: Batch) => ({
          ...batch,
  ...
  const updateModule = (index: number, field: keyof Module, value: string | number) => {
    ...
    setFormData((prev) => prev ? ({ ...prev, modules: updated }) : null);
  };
  ...
  const updateBatch = (index: number, field: keyof Batch, value: string | number | boolean) => {
    ...
    updated[index] = { ...updated[index], [field]: value };
    setFormData((prev) => prev ? ({ ...prev, batches: updated }) : null);
  };
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600">Update course details, modules, and batches</p>
          </div>
        </div>
      </div>
        ...
        {activeTab === 'modules' && (
          <div className="space-y-4">
            {formData.modules.map((module, moduleIndex) => (
              <div key={module._id || moduleIndex} className="bg-white rounded-xl shadow-sm border p-6">
        ...
        {activeTab === 'batches' && (
          <div className="space-y-4">
            {formData.batches.map((batch, index) => (
              <div key={batch._id || index} className="bg-white rounded-xl shadow-sm border p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-semibold">Batch {index + 1}</h3>
                    {batch.enrolledCount !== undefined && batch.enrolledCount > 0 && (
                  ...
                  <button
                    ...
                    className="p-2 text-red-600 hover:bg-red-50 rounded-lg"
                    disabled={batch.enrolledCount !== undefined && batch.enrolledCount > 0}
                    title={batch.enrolledCount ? "Cannot delete batch with enrolled students" : "Delete batch"}
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
